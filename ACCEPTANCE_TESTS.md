...
目前狀態：已有 automated unit/contract/integration coverage，最近一次完整 `npm test` 結果由 `PROJECT_STATE.md` 記錄為 167 tests、166 pass、0 fail、1 個 Electron exported `file://` runtime unavailable skip。這不等於完整 acceptance：真實媒體、完整 desktop `file://`、responsive/touch、error recovery、security review 與真人 Scenario A–G evidence 仍需分層建立。
...
| Layer | 覆蓋 | 目前狀態 |
|---|---|---|
| Unit / contract | model invariants、safe name、relative mapping、validation | IN_PROGRESS |
| Integration | persistence、media pipeline、renderer/export contract | IN_PROGRESS |
| E2E | Projects → Editor → Media → Preview → Export | NOT_STARTED |
| Interaction | keyboard、touch、busy、empty、error、recovery | IN_PROGRESS |
