...
目前狀態：已有 automated unit/contract/integration coverage。標註功能加入前最近一次完整 `npm test` 結果由 `PROJECT_STATE.md` 記錄為 167 tests、166 pass、0 fail、1 個 Electron exported `file://` runtime unavailable skip；2026-08-31 新增的逐幀標註專項測試另有 9 pass、0 fail。由於本輪執行環境無法直接 clone 完整 repository，這 9 個專項測試不得冒充功能加入後的完整回歸。真實媒體、完整 desktop `file://`、responsive/touch、error recovery、security review 與真人 Scenario A–H evidence 仍需分層建立。
...
| Human acceptance | 教練真人使用 Scenario A–H | AWAITING_USER_SETUP |

上述 `IN_PROGRESS` 只表示已有部分 automated evidence；未列出完整、相稱 evidence 前不得改為 `VERIFIED`。
...
1. 解壓 ZIP 到任意一般資料夾。
2. 斷網。
3. desktop modern browser 以 `file://` 開啟 `report.html`。
...
8. 匯出 `report.html` 後，讀者可在本次閱讀自由切換點、線與個別圖層，但不能新增／移動／刪除點，也不回寫 source project。
...
目前 evidence：annotation model/report contract/export runtime/source-level interaction tests 共 9 項通過；完整 repository regression、Electron 真人互動、真實影片 letterbox 與 exported `file://` overlay 尚待補驗，因此 AT-H 為 `IN_PROGRESS`，不是 `VERIFIED`。
