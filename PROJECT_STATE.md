...
- The latest native surface repair is currently uncommitted pending the formal helper rebuild and human reopen check: the HTML anchor no longer paints a second dark layer, the EVR child handles `WM_PAINT`/`WM_ERASEBKGND`, scroll geometry schedules one coalesced repaint after movement settles, and end-of-stream is treated as a paused boundary so replay/scrub does not time out against an ended Media Session. The source links with the available MinGW-w64 toolchain into the project-local `native/bin/media-foundation-player-next.exe`; the formal `media-foundation-player.exe` is left untouched while the user's current helper session is open.
