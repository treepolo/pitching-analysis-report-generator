...
- Launcher stability: `start-pitching-report.bat` now sets `PITCHING_DISABLE_GPU=1`, and `src/main.js` also recognizes an explicit `--disable-gpu` argument before app startup so Chromium uses the in-process GPU path instead of repeatedly spawning a failing GPU/Network pair. Electron smoke now exits successfully; the remaining `os_crypt` line is a non-fatal profile-encryption warning.
