...
- 目前 code integration checkpoint：`46bd9d0 feat: integrate media sync export domain slices`，已整合 `src/media/**`、`src/sync/**`、`src/export/**` 及對應 tests；`00cae62` 為前一個 integration checkpoint。三個 slice 仍只代表可驗證的 contract/domain/export seam，不代表完整產品驗收。
- Recoverable pre-retrofit tag：`checkpoint/pre-governance-retrofit-2026-08-14`。
- 整合 checkpoint 後 tracked implementation files 無未提交修改，原先三個 owner 的 dirty/untracked slice 已納入 `46bd9d0`；私人素材、generated report、ZIP 與 `.tmp` artifacts 未進 Git。
- 本地 checkpoint 可回復被刪除的舊治理檔案。

## Implemented slice

目前已有 Electron shell、隔離的 preload/IPC、project list/create/open/save、project-root persistence、autosave/explicit save/close flush/reopen、最小 section editor、`.txt`/`.md` import persistence、media register/list/remove seam、renderer-only report contract/preview；另已整合 media contract/path policy、read-only signature inspection/realpath containment、pure sync domain，以及帶 allowlist、project-root realpath/symlink boundary、folder/ZIP checksum/parity 與 static file URL validation 的 export slice。這些是 implementation 與 automated evidence，不等於所有 requirement 已驗證。
...
- self-contained report folder、offline `file://`、ZIP/完整交付包與 export consumer 仍未完成產品驗收；目前僅有 fixture-based folder/ZIP seam 與 static contract checks，沒有 real-media/browser/file:// evidence。
...
- `46bd9d0` 的整合 evidence：52 個 `npm test`、25 個 JS `node --check`、media/export/sync targeted tests 39/39、package/lock consistency、`git diff --check`，以及 Electron smoke（含 autosave、explicit save、text import、media list、payload schema、close flush、reopen、bridge security、invalid project rejection）。
