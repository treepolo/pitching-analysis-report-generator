...
- Video block labels are explicit: the block label is the visible upper-left title, each source label is the source title, and the source selector displays the referenced filename. Loop playback has no independent loop bounds; when enabled it uses the source segment's `in`/`out` values. The former loop start/end and relative-offset fields are stripped during save/contract normalization and are not used by runtime synchronization.
