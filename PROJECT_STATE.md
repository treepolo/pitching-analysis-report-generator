...
- The frame player now suppresses legacy inline keyboard handling on frame-player cards, cancels stale exact-seek listeners at the renderer boundary, retargets an active approximate seek instead of stacking `fastSeek` calls, and rejects a timed-out seek as unpresented instead of claiming success. Automated focused tests cover these guards; a manual Electron/media acceptance run is still required for real-frame latency and scroll behavior.
